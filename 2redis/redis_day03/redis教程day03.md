...
一个事务的执行可能是依赖于某个条件 (key)。当条件(key)发生变化,
...
以上代码的执行需要依赖哨兵服务，要保证哨兵服务正常启动。

哨兵案例完成后，如何恢复到最初状态：

1.sudo /etc/init.d/redis-sentinel stop

2.删除哨兵的配置文件

3.找到redis的配置文件，如果有 slaveof 127.0.0.1 xxxx，删除这行。

4.重启服务 sudo /etc/init.d/redis-server restart

5.redis-cli，进入6379的客户端，验证可读写。
